...
function Login() {
  ...
  const onFinish = () => {
    ...
    if (isLogin) {
      request.account.login({ body: { username, password: encryptor.encrypt(password) } }).then(res => {
        navigator('/')
      })
    } else {
      request.account.register({ body: { username, password: encryptor.encrypt(password) } }).then(res => {
