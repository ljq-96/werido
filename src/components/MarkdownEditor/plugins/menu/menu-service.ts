...
import { menuConfig } from './menu-plugin'
...
/// @internal
export type MenuServiceMessageType =
  | {
      type: 'hide'
    }
  | {
...
/// @internal
export type MenuServiceMessage = (message: MenuServiceMessageType) => void
...
export class MenuService {
  ...
  get #filterNodes(): FilterNodes | undefined {
    return this.#ctx?.get(menuConfig.key).filterNodes
  }
  ...
  bind = (ctx: Ctx, notify: MenuServiceMessage) => {
    this.#ctx = ctx
    this.#notify = notify
  }
...
// if (this.invoice.orderCode) {
//   //原
//   // this.$router.push("/pay/payment?rtype=1&currency=" + this.invoice.currency + "&jOrderCode=" + this.invoice.orderCode + "&jInvoiceNo=" + this.invoice.invoiceNo + "&jcompanyCode=" + buyerCompanyCode)
//   // 修改
//   this.$router.push(
//     '/pay/order-list?tab=paymentInvoices&currency=' +
//       this.invoice.currency +
//       '&jOrderCode=' +
//       this.invoice.orderCode +
//       '&jInvoiceNo=' +
...
//   )
// } else {
//   // this.$router.push("/pay/payment?rtype=2&currency=" + this.invoice.currency + "&jOrderCode=" + this.invoice.invoiceCode + "&jInvoiceNo=" + this.invoice.invoiceNo + "&jcompanyCode=" + buyerCompanyCode)
//   this.$router.push(
//     '/pay/payment?rtype=2&currency=' +
//       this.invoice.currency +
//       '&jOrderCode=' +
